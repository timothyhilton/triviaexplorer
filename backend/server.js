  ...
  try {
    ...
    if (!allQuestions) {
      ...
      if (timeSinceLastCall < API_COOLDOWN) {
        ...
        console.log(
          `Proactively waiting ${timeToWait}ms to respect API rate limit.`,
        );
      ...
      if (response.data.response_code === 0 && response.data.results) {
        ...
        if (queryParams.difficulty) {
          questions = questions.filter(
            (q) =>
              ...
              queryParams.difficulty.toLowerCase(),
          );
        }

        if (queryParams.search) {
          questions = questions.filter((q) =>
            q.question.toLowerCase().includes(queryParams.search.toLowerCase()),
          );
        }
        ...
        if (allQuestions.length > 0) {
          console.log(
            `Successfully fetched and transformed ${allQuestions.length} questions. Caching result.`,
          );
          cache.set(cacheKey, allQuestions, 3600); // cache for 1 hour
        } else {
          console.log(
            `Query resulted in no questions after filtering. Not caching.`,
          );
        }
      } else {
        console.log(
          `OpenTDB API returned no results or an error code: ${response.data.response_code}. Caching empty result to avoid repeated failed requests.`,
        );
        allQuestions = [];
    ...
    res
      ...
      .json({
        error: true,
        message:
          "The Trivia Service is currently unavailable. Please try again later.",
      });
  }
});
...
app.get("/api/categories", async (req, res) => {
  ...
  try {
    ...
    if (categories == undefined) {
      console.log("Cache miss for categories. Fetching from OpenTDB...");
      const response = await retry(() =>
        axios.get("https://opentdb.com/api_category.php"),
      );
      if (response.data && response.data.trivia_categories) {
        categories = response.data.trivia_categories;
        cache.set(cacheKey, categories, 86400); // cache for 24 hours
        console.log("Successfully fetched and cached categories.");
      } else {
        throw new Error("Failed to fetch categories from OpenTDB.");
      }
    } else {
    ...
    res
      .status(502)
      .json({ error: true, message: "Could not fetch trivia categories." });
  }
});

const state = {
  ...
  hasMore: true,
  savedQuestions:
    JSON.parse(localStorage.getItem("saved_trivia_questions")) || {},
  isLoading: false,
};
...
function renderCards() {
  ...
  if (state.page === 1) {
    ...
    while (
      questionFeed.firstChild &&
      questionFeed.firstChild !== paginationControls
    ) {
  ...
  newQuestions.forEach((question) => {
    ...
    answersList.addEventListener("click", (e) => {
      ...
      if (!isCorrect) {
        const correctAnswerEl = Array.from(answersList.children).find(
  ...
  elements.loadMoreBtn.style.display =
    state.hasMore && !state.currentFilters.favoritesOnly
      ? "inline-block"
      : "none";
}
...
function updateLoadingState() {
  ...
  if (!state.isLoading) {
    ...
    // dont hide feed if loading more
...
function handleSaveClick(e) {
  ...
  const question = state.questions.find(
    (q) => btoa(unescape(encodeURIComponent(q.question))) === questionId,
  );
  if (!question) return;
  ...
  localStorage.setItem(
    "saved_trivia_questions",
    JSON.stringify(state.savedQuestions),
  );
}
...
function init() {
  ...
  elements.favoritesBtn.addEventListener("click", (e) => {
    ...
    elements.favoritesBtn.classList.toggle(
      "active",
      state.currentFilters.favoritesOnly,
    );
    ...
    if (filtersDisabled) {
      ...
      document
        .querySelector(
          `.difficulty-buttons button[data-difficulty="${currentDifficulty}"]`,
        )
  ...
  document
    .querySelector('.difficulty-buttons button[data-difficulty=""]')
    .classList.add("active");
}
